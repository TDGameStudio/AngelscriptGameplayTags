...
namespace AngelscriptGameplayTags_Private
{
...
TChunkedArray<FGameplayTag, sizeof(FGameplayTag) * 25> GAngelscriptGameplayTags;
...
// since it doesn't break bound pointers used in GAngelscriptGameplayTags.
TSet<FName> GAngelscriptGameplayTagsLookup;
TSet<asIScriptEngine*> GEnginesWithGameplayTagNamespaceGlobals;

asIScriptEngine* GetCurrentScriptEngine()
{
...
void MarkCurrentEngineHasGameplayTagNamespaceGlobals()
{
	if (asIScriptEngine* ScriptEngine = GetCurrentScriptEngine())
	{
		GEnginesWithGameplayTagNamespaceGlobals.Add(ScriptEngine);
	}
}
...
void Bind_AddNewGameplayTag(int32 GameplayTagIndex)
{
	...
	const FGameplayTag& GameplayTag = GAngelscriptGameplayTags[GameplayTagIndex];
...
bool Bind_ProcessGameplayTag(const FName TagName)
{
	...
	if (Tag.IsValid())
	{
		...
		// containers (see AngelscriptEngine.cpp::FAngelscriptEngine::Shutdown
		...
		if (!GAngelscriptGameplayTagsLookup.Contains(TagName))
		{
			Result = true;
			int32 Index = GAngelscriptGameplayTags.AddElement(Tag);
			GAngelscriptGameplayTagsLookup.Add(TagName);
		...
		for (auto& ParentTag : Tag.GetGameplayTagParents())
		{
			...
			if (!GAngelscriptGameplayTagsLookup.Contains(ParentTagName))
			{
				Result = true;
				int32 Index = GAngelscriptGameplayTags.AddElement(ParentTag);
				GAngelscriptGameplayTagsLookup.Add(ParentTagName);
...
bool Bind_AddNewGameplayTags()
{
	...
	if (Result)
	{
		MarkCurrentEngineHasGameplayTagNamespaceGlobals();
	}

	return Result;
}

void ReplayCachedGameplayTagsToCurrentEngine()
{
	asIScriptEngine* ScriptEngine = GetCurrentScriptEngine();
	if (ScriptEngine == nullptr || GEnginesWithGameplayTagNamespaceGlobals.Contains(ScriptEngine))
	{
		return;
	}

	for (int32 TagIndex = 0; TagIndex < GAngelscriptGameplayTags.Num(); ++TagIndex)
	{
...
{
	ReplayCachedGameplayTagsToCurrentEngine();
}
...
void DetachEngine(FAngelscriptEngine& Engine)
{
	...
	{
		GEnginesWithGameplayTagNamespaceGlobals.Remove(ScriptEngine);
	}
}
}
...
{
	if (AngelscriptGameplayTags_Private::Bind_AddNewGameplayTags())
	{
...
{
	AngelscriptGameplayTags_Private::ReplayCachedGameplayTagsToCurrentEngine();
}

void AngelscriptGameplayTagsReplayCurrentEngine()
{
	AngelscriptGameplayTags_Private::ReplayCurrentEngine();
}

void AngelscriptGameplayTagsDetachEngine(FAngelscriptEngine& Engine)
{
	AngelscriptGameplayTags_Private::DetachEngine(Engine);
}
